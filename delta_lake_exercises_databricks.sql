...
DESCRIBE FORMATTED health_tracker_silver;
...
select count(*),p_device_id from health_tracker_silver group by p_device_id;
...
DROP TABLE IF EXISTS late_arriving_data_raw;              

CREATE TABLE late_arriving_data_raw                        
USING json                                             
OPTIONS (
...
select count(*) from late_arriving_data_raw;
...
WHEN MATCHED THEN                                           -- WHEN MATCHED describes the update behavior
  UPDATE SET health_tracker_silver.heartrate = upserts.heartrate   
...
CREATE OR REPLACE TABLE health_tracker_gold
USING DELTA
PARTITIONED BY (p_device_id)
LOCATION "/health_tracker/gold" 
AS
(select avg(heartrate) avg_heartrate,stddev(heartrate) stddev_heartrate,p_device_id
